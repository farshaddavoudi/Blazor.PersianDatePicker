function blazorReady(
    ...
    $('#' + elementId + '.blazor-datepicker').each(function () {
        ...
        const mapDigitConfig = function (type, isPersianCalendar, isExplicit) {
            if (type === 'en') {
                return { usePersianDigits: false, locale: 'en' };
            }
            if (type === 'fa') {
                return { usePersianDigits: true, locale: 'fa' };
            }
            if (isExplicit && type !== 'float') {
                return isPersianCalendar
                    ? { usePersianDigits: true, locale: 'fa' }
                    : { usePersianDigits: false, locale: 'en' };
            }
            return isPersianCalendar ? { usePersianDigits: true, locale: 'fa' } : { usePersianDigits: false, locale: 'en' };
        };

        const ensureDigitMode = function () {
            const pickerInstance = $datePicker.pDatePicker;
            if (!pickerInstance || !pickerInstance.model || !pickerInstance.model.options) {
                return;
            }
            ...
            if (pickerInstance.model.options.persianDigit !== pickerConfig.usePersianDigits) {
                pickerInstance.model.options.persianDigit = pickerConfig.usePersianDigits;
                optionsChanged = true;
            }

            if (pickerInstance.model.options.calendar) {
                if (pickerInstance.model.options.calendar.persian && pickerInstance.model.options.calendar.persian.locale !== pickerConfig.locale) {
                    pickerInstance.model.options.calendar.persian.locale = pickerConfig.locale;
                    optionsChanged = true;
                }
                if (pickerInstance.model.options.calendar.gregorian && pickerInstance.model.options.calendar.gregorian.locale !== 'en') {
                    pickerInstance.model.options.calendar.gregorian.locale = 'en';
                }
            }

            if (pickerInstance.model.options.locale_ !== pickerConfig.locale) {
                pickerInstance.model.options.locale_ = pickerConfig.locale;
                optionsChanged = true;
            }

            if (pickerInstance.model.PersianDate && typeof pickerInstance.model.PersianDate.toLocale === 'function') {
                pickerInstance.model.PersianDate.toLocale(pickerConfig.locale);
            }

            if (typeof persianDate !== 'undefined'
                && persianDate
                && typeof persianDate.toLocale === 'function'
                && typeof persianDate.toCalendar === 'function') {
                persianDate.toCalendar(activeCalendar);
                persianDate.toLocale(pickerConfig.locale);
            }

            const rawValue = $datePicker.val();
            if (rawValue) {
                const normalizedValue = inputConfig.usePersianDigits
                    ? toPersianNum(rawValue, true)
                    : toEnglishNum(rawValue, true);
                if (normalizedValue !== rawValue) {
                    $datePicker.val(normalizedValue);
                }
            }

            if (optionsChanged && pickerInstance.model.view && typeof pickerInstance.model.view.render === 'function') {
                pickerInstance.model.view.render();
            }
        };

        const initialPickerConfig = mapDigitConfig(pickerDigitType, calendarType === 'persian', hasExplicitPickerDigitType);

        $datePicker.persianDatepicker(
            {
                calendarType: calendarType,
                persianDigit: initialPickerConfig.usePersianDigits,
                calendar: {
                    persian: {
                        locale: initialPickerConfig.locale,
                        showHint: false,
